...
function getStatus(value: number, warning: number, critical: number): ValueStatus {
...
export function StateBar({ state }: Props) {
  ...
  const approvalVal = typeof approval === "number" ? `${Math.round(approval)}%` : "-";
  const firewood = typeof fuel?.firewood === "number" ? Math.round(fuel.firewood) : "-";
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-3 bg-card border border-border rounded-[var(--radius)]">
          ...
          <ResourceGroup
            ...
            items={[
              ...
              { label: "Homeless", value: pop.homeless ?? 0, status: (pop.homeless ?? 0) > 0 ? "warning" : "normal" },
              { label: "Wealth", value: state.settlement?.regional_wealth ?? 0 },
            ...
            items={[
              {
                ...
                status: typeof fuel?.firewood === "number" ? getStatus(fuel.firewood, 15, 5) : "normal",
              },
            ...
            <ResourceGroup
              ...
              items={[
                { label: "Retinue", value: military.retinue_count ?? 0 },
                {
                  ...
                  status: (military.bandit_camps_nearby ?? 0) > 0 ? "warning" : "normal",
                },
              ]}
            />
          )}
