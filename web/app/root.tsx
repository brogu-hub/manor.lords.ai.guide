  ...
  return (
    <html lang="en" className="dark">
      <head>
        ...
        <title>Manor Lords Advisor</title>
        <Meta />
        <Links />
      </head>
      <body>
...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  return (
    <main className="pt-16 p-4 container mx-auto">
      <h1 className="font-heading text-primary text-2xl">{message}</h1>
      <p className="text-muted-foreground mt-2">{details}</p>
      {stack && (
        <pre className="w-full p-4 overflow-x-auto mt-4 text-sm bg-card rounded">
          <code>{stack}</code>
        </pre>
      )}
    </main>
